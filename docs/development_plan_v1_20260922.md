...
| 第二阶段：箱号校验位优化 | 代码完成，待P3正式API回归 | 英文模型独立实验使固定开发集达到89/89；0.4.0已接入正式图片API |
...
没有错误验证。固定89张开发集累计达到89/89。0.4.0已将该策略接入正式图片API：只在
通用OCR第一候选为合法前10位且状态为`unverified`时触发，三个变体的选择过程不读取
正确答案和ISO计算值；模型异常时保留原始10位结果。下一项工作是在P3重新构建0.4.0，
通过正式API从89张原始照片完整回归，并准备100～200张未参与调试的新照片盲测。
...
0.4.0中，如果箱号通用OCR第一候选只有前10位，接口会运行校验位后备识别。结果保存在
`results.container_number.postprocessing`，包含执行状态、模型、三个变体的原始文字和
置信度、选择原因、裁剪坐标及最终验证状态。`postprocessing`只用于审计，不改变
...
DAS_AI_CHECK_DIGIT_MIN_SINGLE_SCORE=0.70
```
