...
| 第二阶段：铅封错误样本优化 | P3回归通过 | 三方向推理、候选融合和自动排序已接入图片API，正式API结果43/44 |
| 第二阶段：箱号校验位优化 | 0.4.1代码完成，待P3回归 | 0.4.0正式API为85/89且无错误验证；0.4.1增加上下文OCR复核和边界阈值修正 |
| 第三阶段：DAS Photo页面接入 | 未开始 | 等P3图片推理稳定后开发 |

因此，本项目目前处于“第二阶段识别策略已完成代码接入，等待P3正式API全量回归和新
照片盲测”的位置。箱号建议值不能代替图片观察值，箱号优化不会被遗漏或并入铅封逻辑。
...
P3实测在这12张中救回6张，组合累计达到83/89（93.26%），没有错误验证。单独校验位
裁剪是本轮效果最好的变体，6张未救回图片的裁剪范围也都包含完整数字，但OCR检测阶段
返回空文字；其中三个校验位为带方框的窄字符`1`，说明下一步应验证“文字检测失败”而
不是继续扩大原图裁剪。

`v0.3.3`新增独立评估脚本`scripts.evaluate_check_digit_recognition`，直接调用PaddleOCR
`TextRecognition`模块，对v0.3.2保存的校验位小图执行三路识别：保留方框彩色、保留方框
灰度增强、去方框灰度增强。直接识别绕过文本检测，只处理已经定位的文字行；结果融合
不读取正确答案，也不使用ISO计算值选择数字。两个以上变体一致时自动采用；只有一个
数字时要求高置信度；不同数字冲突时进入人工确认。该脚本先在P3评估，不接入正式图片
API，避免未经验证的预处理影响现有服务。

P3对最后6张使用通用`PP-OCRv5_server_rec`时救回5张；使用英文模型
`en_PP-OCRv5_mobile_rec`后6张全部正确，`inner_gray`单变体6/6，平均置信度0.990724，
没有错误验证。固定89张开发集的分步实验累计达到89/89。0.4.0随后把该策略接入正式
图片API：只在通用OCR第一候选为合法前10位且状态为`unverified`时触发，三个变体的
选择过程不读取正确答案和ISO计算值；模型异常时保留原始10位结果。

0.4.0正式API全量回归得到85/89，错误验证为0；后处理`not_needed`31张、`applied`55张、
`low_confidence`2张、`not_found`1张。剩余4张分别暴露三类问题：通用上下文OCR原本能够
识别但直接单字符识别失败；正确单数字置信度0.663281和0.672433略低于0.70阈值；以及
两个方框变体对错误数字形成共识，但被ISO校验正确标为`mismatch`。铅封仍为43/44，未
发生回退。

0.4.1采用分层复核：先运行普通OCR，再运行英文校验位直接识别；如果直接识别没有获得
`verified`结果（包括`not_found`、`low_confidence`、`conflict`或`mismatch`），则复用已
加载的通用OCR模型，对已验证有效的`context_2x`、`context_4x`和`grayscale_4x`三个上下文
裁剪重新识别。上下文结果只有在独立观察到完整11位、前10位与原候选完全一致且ISO校验
通过时才可采用。单一清晰数字阈值从0.70调整为0.65；这只覆盖本次两张边界样本，ISO仍
只负责事后验证，不能生成或替换OCR数字。下一项工作是在P3运行0.4.1的89张箱号和44张
铅封正式API回归，确认错误验证仍为0，再准备100～200张未参与调试的新照片盲测。
...
0.4.1中，如果箱号通用OCR第一候选只有前10位，接口会先运行校验位直接识别；直接结果
没有通过ISO验证时，再运行三个上下文裁剪的通用OCR复核。结果保存在
`results.container_number.postprocessing`，包含执行状态、模型、三个变体的原始文字和
置信度、选择原因、裁剪坐标、上下文复核明细及最终验证状态。`postprocessing`只用于审计，不改变
`ocr_items`中通用OCR的原始输出。
...
`create_engine()`对通用OCR实例做缓存，`create_check_digit_recognizer()`对英文校验位模型
做独立缓存。两个预测器共用一个GPU运行锁。生产容器设置：

```env
DAS_AI_ENGINE=paddle_gpu
DAS_AI_PRELOAD_MODEL=1
DAS_AI_CONTAINER_CHECK_DIGIT_FALLBACK=1
```

服务启动时加载通用OCR和英文校验位两个模型；如果模型或GPU不可用，容器启动失败，从而
避免表面在线但实际不能识别。
...
DAS_AI_CONTAINER_CHECK_DIGIT_FALLBACK=1
DAS_AI_CHECK_DIGIT_MODEL=en_PP-OCRv5_mobile_rec
DAS_AI_CHECK_DIGIT_MIN_CANDIDATE_SCORE=0.50
DAS_AI_CHECK_DIGIT_MIN_SINGLE_SCORE=0.65
```
...
13. 校验位直接识别的严格单数字清洗、多变体共识、高置信度单结果和冲突保留。
14. 正式图片API仅在10位未验证候选上触发后备识别，并在失败时保留主OCR结果。
15. 通用OCR与校验位识别模型共享GPU推理锁，避免并发预测器同时占用GPU。
...
7. 在固定89张箱号开发集上完成局部裁剪/放大和单字符直接识别；（独立实验已达到89/89，待正式API全量回归）
8. 建立100～200张未参与调试的新照片盲测集，先冻结人工正确答案再运行模型。（未开始）
...
### v1.5 / 软件0.3.0（校验位直接识别实验）

- P3的v0.3.2定向实验在12张困难样本中救回6张，固定集累计达到83/89；
- 确认剩余6张的校验位完整落在裁剪区内，主要故障点是文字检测未返回孤立数字；
- 新增PaddleOCR `TextRecognition`单字符直接识别评估，绕过文字检测；
- 新增保留方框彩色、保留方框灰度增强、去方框灰度增强三个变体；
- 新增不读取正确答案的结果融合：优先多变体共识，其次高置信度单结果，冲突则人工确认；
- ISO 6346仍只负责事后验证，不负责生成、纠正或选择OCR观察数字。

### v1.6 / 软件0.4.0（正式API接入）

- 英文`en_PP-OCRv5_mobile_rec`在最后6张困难样本上达到6/6，固定开发集累计89/89；
- 把校验位彩色、灰度和去框灰度三变体直接识别接入正式图片API；
- 仅对通用OCR的10位`unverified`第一候选触发，不重复处理完整11位结果；
- 多变体融合不读取正确答案，ISO 6346仅对已经选择的OCR数字做事后验证；
- 后备模型异常时保留原始10位结果，并通过`postprocessing`返回可审计状态；
- 通用OCR与英文校验位模型使用共享GPU锁，Compose启动时预加载两个模型；
- 批量评估汇总新增后备流程状态统计；
- 下一步在P3运行0.4.0正式API的89张箱号和44张铅封全量回归，然后建立新照片盲测集。

### v1.7 / 软件0.4.1（分层复核）

- 0.4.0正式API回归箱号85/89，错误验证为0，铅封43/44；
- 将单一清晰数字自动采用阈值从0.70调整到0.65，覆盖两张正确结果刚好低于旧阈值的样本；
- 直接校验位识别未验证或产生`mismatch`时，增加三个上下文裁剪的通用OCR复核；
- 上下文结果必须独立观察到完整11位、前10位完全一致且ISO校验通过，才允许覆盖直接结果；
- 上下文OCR复用已加载的通用模型，不增加第三个模型，也不读取正确答案；
- 下一步在P3运行0.4.1完整133张回归；通过后建立100～200张新照片盲测集。
